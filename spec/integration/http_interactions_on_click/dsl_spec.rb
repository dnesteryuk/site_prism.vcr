...
feature 'Http interactions on click > DSL' do
  let(:cat_owner)     { test_app_page.cat_owner }
  let(:test_app_page) { DslPage.new }
  ...
  it_behaves_like 'clicks and handles one HTTP request'
  ...
  context 'when an user clicks on the link which do 2 HTTP requests' do
    before do
